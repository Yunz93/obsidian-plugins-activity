...
describe("releaseNotes", () => {
  it("includes 0.2.2 with non-empty en and zh highlights", () => {
    const entry = RELEASE_NOTES.find((note) => note.version === "0.2.2");
  ...
  it("returns localized highlights for a known version", () => {
    expect(getReleaseNotes("0.2.0", "en").length).toBeGreaterThan(0);
    expect(getReleaseNotes("0.2.0", "zh").length).toBeGreaterThan(0);
  });
